    ...
    <ul>
      ...
      <li class="good-con" v-if="buyOrderInfo.showGoodsList&&buyOrderInfo.showGoodsList.length!=0">
        ...
        <div  class="good-list list" v-for="(buyGoods,index) in buyOrderInfo.showGoodsList" v-if="buyGoods.quantity!=0">
          ...
          <span class="list-option iconfont icon-jian jian-goods" v-if="buyOrderInfo.status!=2&&buyOrderInfo.status!=3" @click="$set(buyGoods,'quantity',0)"></span>
        </div>
      </li>
    </ul>
    <ul>
...
<script>
  ...
  import {savebuyorder,getbuyorder,deletebuyorder,cancelbuyorder} from 'src/service/getData'
    ...
    methods: {
      ...
      goGoods(){
        if(this.buyOrderInfo.warehouseId){
          if(!this.buyOrderInfo.status || this.buyOrderInfo.status!=2&& this.buyOrderInfo.status!=3){
            this.$router.push({name:'choosegoods'})
          }
        }else{
          this.showTip('请先选择仓库！')
        }
      ...
      getBuyOrder(){
        //编辑采购单时获取信息
        getbuyorder(this.$route.query.id).then((res)=>{
      ...
      async submitOrder(status){
        ...
        // let buyGoods = []
        // this.buyOrderInfo.buyGoods.forEach(element => {
        ...
        savebuyorder(this.userId,submitOrder).then((res)=>{
          this.returnBack();
        }).catch((err)=>{
      ...
      cancel(){
        //撤销已采购订单
        cancelbuyorder(this.userId,this.buyOrderInfo.id).then((res)=>{
          this.returnBack()
        }).catch((err)=>{
           this.showTip(err.message)
        })
      ...
      deleteOrder(){
        //删除草稿订单
        deletebuyorder(this.buyOrderInfo.id).then((res)=>{
          this.returnBack()
        }).catch((err)=>{
          this.showTip(err.message)
        })
